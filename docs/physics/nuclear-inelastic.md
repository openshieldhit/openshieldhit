...
osh_nuclear_tripathi       parametric reaction cross section σ_R (Tripathi 1999)
osh_nuclear_sigma_reac     σ_R entry point: evaluated tables (O-16, C-12) + Tripathi fallback
...
## Cross sections (issue #277)

**Reaction (nonelastic) σ_R** — `osh_nuclear_sigma_reac()`.  For protons on
O-16 and C-12 the transport interpolates condensed evaluated tables
(ENDF/B-VIII.0 proton sublibrary, the LA150h evaluation behind the ICRU 63
nonelastic tables; flat clamp above its 150 MeV end, supported by the
Renberg 1972 measurements).  The Tripathi parameterisation — which
overpredicts on these targets by up to +13% (O) / +24% (C) near 100 MeV —
remains the fallback for all other targets and projectiles.  The tables are
generated by `tools/condense_pa_xsec.py` from the committed reference data in
`tests/reference/xsec/` (fetched from IAEA NDS by `tools/fetch_pa_xsec.py`).

**p+A elastic σ_el** — `osh_nuclear_elastic_sigma()`.  σ_el = r(E)·σ_R with
an energy-dependent ratio replacing the earlier constant black-disk factor:
r = 1 at and below 30 MeV, 0.32 at and above 180 MeV, log-E interpolated
between (compile-time anchors `OSH_NUCLEAR_ELASTIC_RATIO_*`).  Calibrated
against the only integrated (P,EL) measurement on a therapy target (Garron
1962, p+C-12 at 155 MeV: 75 ± 7 mb; the model gives 84 mb) and validated at
transport level against the SH12A primary depth-fluence fixture.  Note that
the MF3/MT2 "elastic" entry of charged-particle evaluations is a
...
```text
σ_R hazard (tables/Tripathi)  →  struck-element selection (per-nuclide rates)
...
- `examples/06_nucre_validation/sigma_scan` + `tools/plot_xsec_pa.py` —
  the transport's σ_R/σ_el curves against the committed EXFOR/evaluated
  reference data (`tests/reference/xsec/`) and the SH12A-implied removal
  rate.
...
- Tripathi, Cucinotta, Wilson, NASA/TP-1999-209726 — parametric reaction
  cross section (fallback targets).
- Chadwick et al., LA150 / ENDF/B-VIII.0 proton sublibrary; ICRU Report 63
  (2000) — evaluated nonelastic cross sections for O-16, C-12.
- Garron et al., Nucl. Phys. 37 (1962) 126 (EXFOR O0360005) — integrated
  nuclear elastic p+C at 155 MeV, σ_el/σ_R anchor.
- Renberg et al., Nucl. Phys. A 183 (1972) 81 — σ_R above 200 MeV
  (flat-clamp support).
