...
When a scoring page enables variance tracking (a `Variance On` `Settings` block on
its `Quantity` line), `--score-replicas <n>` doubles as an **`n`-batch
standard-error source**: each replica is folded as one
