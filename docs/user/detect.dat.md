...
A global `VARIANCE` card turns on a per-bin Monte-Carlo **standard error** for
every scored quantity:

```text
VARIANCE          # error bars from the default number of batches
VARIANCE 20       # error bars from 20 batch-means observations
```

Place it at column 0 (not inside a section); it applies to the whole run.

**How it works.** The run is split into `N` contiguous, equal blocks of primary
...
batches to report anything — `VARIANCE 1` (or a run with no batching) has zero
degrees of freedom and its error columns are all zeros.  Because each history's
random stream is a pure function of its global index, a fixed batch count is
...
**Cost & scope.** Enabling `VARIANCE` roughly doubles the scoring memory (a
companion sum-of-squares array per accumulator) and adds one merge per batch
boundary; it does not change the scored values beyond floating-point summation
order.  It is currently written by the `TEXT`/`ASCII` writer only — the `BDO2019`
writer still emits the values (its standard-error field is a planned addition).
...
diagnostic, uses those batches instead of the `VARIANCE` default.
