...
static void test_prepare_mixed_loaddedx_and_bethe_paths(void) {
    ...
    ASSERT_NEAR(osh_transport_sp_lookup(&tables, OSH_MATERIAL_INDEX_BLACKHOLE, 0u, OSH_TRANSPORT_EMIN), 0.0, 1e-12);
    ...
    ASSERT_NEAR(osh_transport_sp_lookup(&tables, idx_table20, 19u, 0.025), 1.0, 1e-6);
