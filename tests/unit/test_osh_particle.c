...
void test_isotope_db(void) {
    struct isotope const *iso = &osh_isotope_db[osh_isotopes_idx_default[Z_OSMIUM]];
    assert(iso->z == 76);
    assert(iso->a == 192);
...
void test_particles(void) {
    ...
    assert(osh_particle_from_pdg(&p, OSH_PART_PDG_PROTON) == 1);
...
void test_particle_name_from_pdg(void) {
    ...
    assert(osh_particle_name_from_pdg(OSH_PART_PDG_PROTON, buf, sizeof(buf)) == 1);
    assert(strcmp(buf, "proton") == 0);

    assert(osh_particle_name_from_pdg(OSH_PART_PDG_DEUTERON, buf, sizeof(buf)) == 1);
    assert(strcmp(buf, "deuteron") == 0);

    assert(osh_particle_name_from_pdg(OSH_PART_PDG_HE4, buf, sizeof(buf)) == 1);
    assert(strcmp(buf, "He-4") == 0);

    /* ion not in particle_db: falls through to isotope_db -> "C-12" */
    assert(osh_particle_name_from_pdg(PDG_CARBON_12, buf, sizeof(buf)) == 1);
    assert(strcmp(buf, "C-12") == 0);

    /* unknown PDG: returns 0 and nulls the buffer */
    buf[0] = 'x';
    assert(osh_particle_name_from_pdg(999, buf, sizeof(buf)) == 0);
    assert(buf[0] == '\0');

    /* buf_size == 0: returns 0 without touching buffer */
    assert(osh_particle_name_from_pdg(OSH_PART_PDG_PROTON, buf, 0) == 0);
}
...
void test_particle_symbol_from_pdg(void) {
    ...
    /* named particles resolved via particle_db */
    assert(osh_particle_symbol_from_pdg(OSH_PART_PDG_PROTON, buf, sizeof(buf)) == 1);
    assert(strcmp(buf, "p") == 0);

    assert(osh_particle_symbol_from_pdg(OSH_PART_PDG_DEUTERON, buf, sizeof(buf)) == 1);
    assert(strcmp(buf, "d") == 0);

    assert(osh_particle_symbol_from_pdg(OSH_PART_PDG_HE4, buf, sizeof(buf)) == 1);
    assert(strcmp(buf, "He4") == 0);

    /* ion not in particle_db: falls through to isotope_db -> "C-12" */
    assert(osh_particle_symbol_from_pdg(PDG_CARBON_12, buf, sizeof(buf)) == 1);
    assert(strcmp(buf, "C-12") == 0);

    /* unknown PDG: returns 0 and nulls the buffer */
    buf[0] = 'x';
    assert(osh_particle_symbol_from_pdg(999, buf, sizeof(buf)) == 0);
    assert(buf[0] == '\0');

    /* buf_size == 0: returns 0 without touching buffer */
    assert(osh_particle_symbol_from_pdg(OSH_PART_PDG_PROTON, buf, 0) == 0);
}
...
void test_particle_from_pdg_ion(void) {
    ...
    assert(osh_particle_from_pdg(&p, OSH_PART_PDG_DEUTERON) == 1);
    ...
    assert(osh_particle_from_pdg(&p, OSH_PART_PDG_HE4) == 1);
    assert(p.is_nucleus == 1);
    assert(p.z == 2);
    assert(p.a == 4);
    assert(p.charge == 2);
    ...
    assert(osh_particle_from_pdg(&p, PDG_CARBON_12) == 1);
    assert(p.is_nucleus == 1);
    assert(p.z == 6);
    assert(p.a == 12);
    assert(p.charge == 6);
...
void test_particle_pdg_from_name(void) {
    ...
    assert(osh_particle_pdg_from_name("proton", &pdg) == 1);
    assert(pdg == OSH_PART_PDG_PROTON);

    assert(osh_particle_pdg_from_name("P", &pdg) == 1);
    assert(pdg == OSH_PART_PDG_PROTON);

    assert(osh_particle_pdg_from_name("antiproton", &pdg) == 1);
    assert(pdg == OSH_PART_PDG_APROTON);

    assert(osh_particle_pdg_from_name("alpha", &pdg) == 1);
    assert(pdg == OSH_PART_PDG_HE4);

    assert(osh_particle_pdg_from_name("no_such_particle", &pdg) == 0);
}

void test_particle_from_name(void) {
    struct particle p;

    assert(osh_particle_from_name(&p, "He4") == 1);
