...
static void test_icru_air_dry(void) {
    ...
    /* Nitrogen dominates by mass */
    {
        ...
        for (i = 0; i < e.nelements; i++) {
            if (e.elements[i].z == 7u) mfrac_n = e.elements[i].mass_fraction;
            if (e.elements[i].z == 8u) mfrac_o = e.elements[i].mass_fraction;
...
static void test_icru_pmma(void) {
    ...
    ASSERT_TRUE(e.nelements == 3u);
    {
        ...
        double mfrac_o = -1.0;
        for (i = 0; i < e.nelements; i++) {
            if (e.elements[i].z == 1u) mfrac_h = e.elements[i].mass_fraction;
            if (e.elements[i].z == 6u) mfrac_c = e.elements[i].mass_fraction;
            if (e.elements[i].z == 8u) mfrac_o = e.elements[i].mass_fraction;
        }
