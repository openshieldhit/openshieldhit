...
/* Schneider 2000 breakpoints (lower bin edges):
 * {-1000,-950,-120,-82,-52,-22,8,19,80,120,...,1600}
 * Bin index = lower bracket index in _ct_hu[].
 *   HU=-1000 → bin 0 (air)
 *   HU=0     → falls in [-22, 8) → bin 5
 *   HU=1600  → bin 23 (upper limit, last bin)
 */

/* Verify osh_voxel_hu2rho_schneider2000 is continuous and monotonically
 * increasing across breakpoints.  Checks that the Schneider 2000 piecewise
 * fits join without jumps at HU = -98, 14, 23, 100. */
static void test_hu2rho_continuity(void) {
    float rho_lo;
    float rho_hi;

    /* breakpoint at -98: air/lung → adipose-water */
    rho_lo = osh_voxel_hu2rho_schneider2000(-99);
    rho_hi = osh_voxel_hu2rho_schneider2000(-97);
    ASSERT_TRUE(rho_hi > rho_lo); /* density increases with HU */

    /* breakpoint at 14: adipose-water → water transition */
    rho_lo = osh_voxel_hu2rho_schneider2000(13);
    rho_hi = osh_voxel_hu2rho_schneider2000(15);
    ASSERT_TRUE(rho_hi > rho_lo);

    /* air density at HU = -1000 */
    ASSERT_TRUE(osh_voxel_hu2rho_schneider2000(-1000) < 0.002f); /* ~0.00121 g/cm³ */

    /* water at HU = 0 */
    {
        float rho_water = osh_voxel_hu2rho_schneider2000(0);
        ASSERT_TRUE(rho_water > 0.99f && rho_water < 1.05f);
    }

    /* bone at HU = 1000 */
    ASSERT_TRUE(osh_voxel_hu2rho_schneider2000(1000) > 1.5f);
}

static void test_hu_lut_boundary_values(void) {
    uint8_t lut[OSH_VOXEL_HU_LUT_SIZE];

    osh_voxel_build_hu_bin_lut_schneider2000(lut);
...
static void test_hu_lut_matches_hu2idx(void) {
    ...
    hu = -1000;
    while (hu <= 1600) {
        ASSERT_TRUE((int) lut[hu + 1000] == osh_voxel_hu2idx_schneider2000((int16_t) hu));
        hu++;
    }
}
...
static void test_permatassari_hu_lut_boundary_values(void) {
    ...
    osh_voxel_build_hu_bin_lut_permatassari2020(lut);
...
static void test_permatassari_hu2rho_reference_point(void) {
    ...
    rho = osh_voxel_hu2rho_permatassari2020(0, 14);
    ASSERT_TRUE(rho > 0.997f && rho < 0.999f); /* expected ~0.998 */

    /* The first bin can go near zero in TOPAS; our implementation clamps it. */
    ASSERT_TRUE(osh_voxel_hu2rho_permatassari2020(-1000, 0) >= 0.0001f);
}

static void test_rho_lut_schneider_values(void) {
    float lut[OSH_VOXEL_HU_LUT_SIZE];

    osh_voxel_build_hu_rho_lut_schneider2000(lut);
    ...
    /* LUT matches the scalar function at every point */
    {
        ...
        for (hu = -1000; hu <= 1600; hu++) {
            ASSERT_TRUE(fabsf(lut[hu + 1000] - osh_voxel_hu2rho_schneider2000((int16_t) hu)) < 1.0e-6f);
        }
    }
}
...
static void test_rho_lut_permatassari_values(void) {
    ...
    osh_voxel_build_hu_rho_lut_permatassari2020(lut);
