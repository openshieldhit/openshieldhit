...
static void print_projectile_slice(struct osh_material_runtime const *tables,
                                   size_t mat_idx,
                                   size_t proj_idx,
                                   char const *label,
    ...
    for (i = 0; i < nenergies; ++i) {
        ...
        e = energies[i];
        sp = osh_material_runtime_sp_lookup(tables, mat_idx, proj_idx, e);
        range = osh_material_runtime_range_lookup(tables, mat_idx, proj_idx, e);
        printf("  E=%9.4f MeV/u  dEdx=%12.6f MeV cm^2/g  range=%12.6f g/cm^2\n", e, sp, range);
    }
}

static void print_projectile_slice_with_diff(struct osh_material_runtime const *tables,
                                             size_t mat_idx,
                                             size_t ref_mat_idx,
                                             size_t proj_idx,
                                             char const *label,
    ...
    for (i = 0; i < nenergies; ++i) {
        ...
        sp = osh_material_runtime_sp_lookup(tables, mat_idx, proj_idx, e);
        range = osh_material_runtime_range_lookup(tables, mat_idx, proj_idx, e);
        ref_sp = osh_material_runtime_sp_lookup(tables, ref_mat_idx, proj_idx, e);
        ref_range = osh_material_runtime_range_lookup(tables, ref_mat_idx, proj_idx, e);
        d_sp = sp - ref_sp;
