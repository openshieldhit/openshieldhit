...
static void test_init_disables_everything(void) {
    ...
    ASSERT_TRUE(ctl.stop_flag == NULL);
    ASSERT_TRUE(ctl.dump_flag == NULL);

    /* Zeroed control: unlimited budget, no flag → never stops. */
...
static void test_stop_flag(void) {
    struct osh_run_control ctl;
    sig_atomic_t volatile flag = 0;

    osh_run_control_init(&ctl);
    ctl.stop_flag = &flag;

    /* Flag clear, no budget: keep going. */
    ASSERT_TRUE(run_ctl_should_stop(&ctl, 5.0, 7u) == 0);

    /* Raise the flag: stop regardless of elapsed time. */
    flag = 1;
    ASSERT_TRUE(run_ctl_should_stop(&ctl, 0.0, 0u) == 1);

    /* Lowering it again returns to "keep going". */
    flag = 0;
    ASSERT_TRUE(run_ctl_should_stop(&ctl, 0.0, 0u) == 0);
}

static void test_budget_and_flag_combine_or(void) {
    struct osh_run_control ctl;
    sig_atomic_t volatile flag = 0;
    ...
    /* Flag alone, well under budget. */
    flag = 1;
    ASSERT_TRUE(run_ctl_should_stop(&ctl, 1.0, 0u) == 1);

    /* Budget alone, flag clear. */
    flag = 0;
...
int main(void) {
    ...
    test_stop_flag();
    test_budget_and_flag_combine_or();
