#include "species/osh_particle_db.h"

#include <stddef.h>
#include "common/osh_const.h"
#include "species/osh_particle_pdg.h"
...
*/
struct particle_db_type const osh_particle_db[] = {
    { MEV(1.008664916000e+00), "neutron", "n", OSH_PART_PDG_NEUTRON, 0 },
    { MEV(1.007276466812e+00), "proton", "p", OSH_PART_PDG_PROTON, 1 },
    ...
    { MEV(1.008664916000e+00), "antineutron", "n~", OSH_PART_PDG_ANEUTRON, 0 },
    { MEV(1.007276466812e+00), "antiproton", "p~", OSH_PART_PDG_APROTON, -1 },
    ...
    { MEV(5.485799082992e-04), "electron", "e-", OSH_PART_PDG_ELECTRON, -1 },
    { MEV(5.485799082992e-04), "positron", "e+", OSH_PART_PDG_POSITRON, 1 },
...
struct osh_particle_db_entry const *osh_particle_db_get(int pdg)
{
    for (size_t i = 0; i < osh_particle_db_len; ++i) {
        if (osh_particle_db[i].pdg == pdg) return &osh_particle_db[i];
    }
    return NULL;
}