...
int osh_readline_key(struct oshfile *oshf, char **lline, char **kkey, char **aargs, int *lineno) {
    ...
    while (fgets(buff, OSH_MAX_LINE_LENGTH, oshf->fp)) {
        ...
        i = 0;
        while (isspace((unsigned char)buff[i])) i++;
        if (buff[i] == '\0' || _is_comment(buff[i])) continue;
        ...
        *kkey = line + i;
        key_end = i;
        while (line[key_end] && !isspace((unsigned char)line[key_end]) && !_is_comment(line[key_end])) key_end++;
        if (line[key_end]) line[key_end] = '\0';

        /* Find args */
        args_start = key_end + 1;
        while (line[args_start] && isspace((unsigned char)line[args_start])) args_start++;
        if (line[args_start] == '\0' || _is_comment(line[args_start])) {
            ...
            *aargs = line + args_start;
            ...
            while (line[j] && !_is_comment(line[j])) {
                if (!isspace((unsigned char)line[j])) last_char = j;
                j++;
            }
            line[last_char + 1] = '\0';
        }
