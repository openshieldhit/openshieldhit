...
void osh_vect_orthogonal_basis(double const *w, double *u, double *v) {
    ...
    /*
       ...
       If |u| > 0 then u and e3 span a plane in 3D space and v is well defined normal vector.
       It is safe here to compare with zero, as this is in fact check for non-zero vector
       afterwards only multiplications are used, so even if `u` has a small norm, the numerical error won't propagate.
     */
    if (osh_vect_len2(u) > 0) {
        ...
        /*
           If u is oriented in the same direction as e1 (u==e1, as both are unit vectors)
            then we create a standard basis (u=e1, v=e2, w=e3).
           If u is oriented in opposite direction as e1(u == -e1, as both are unit vectors)
            then we choose another basis which has the same orientation as standard basis (u=-e1, v=-e2, w=e3).
         */
        for (i = 0; i < OSH_VECT_DIM; i++) {
            u[i] = sign * im[0][i];
            v[i] = im[1][i];
...
void osh_vect_setup_tmatrix_bzalign(double *p, double *r, double *tm) {
    ...
    /* here we need to use R as last basis vector, therefore to maintain correct orientation */
    /* we choose STR order */
