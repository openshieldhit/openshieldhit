...
/** @brief Index reference from a page to a compiled filter. */
struct osh_scoring_page_filter_ref {
    size_t filter_idx; /* Index into osh_scoring_runtime::filters[]. */
};
...
struct osh_scoring_page_runtime {
    ...
    struct osh_scoring_page_filter_ref *filters;    /* Filter index references (owned). */
    ...
    size_t nfilters;                                /* Number of filter references. */
