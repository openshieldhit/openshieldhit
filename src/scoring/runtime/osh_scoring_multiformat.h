...
/**
 * @brief Fan a multi-format `Output` block out into one runtime output per format.
 *
 * @details
 ...
 * first requested format and a verbatim filename.  This phase expands a block
 * that requests several formats into one extra runtime output per additional
 * format, each sharing the block's `page_indices` — a cheap `size_t` copy;
 * nothing in `rt->pages[]` is duplicated, so scoring memory is independent of the
 * format count (issue #308).
 *
 * With more than one format the block's `Filename` becomes a stem and each target
 * gets a canonical extension (or an explicit per-target override); a single
 * format keeps the filename verbatim, so every existing `detect.dat` is
 * unaffected.  RTDOSE targets are reduced to a single page (see the
 * single-format vs mixed-block rules documented in `docs/user/detect.dat.md`).
 *
