...
static struct xsec_entry const k_jeff[] = {
    ...
    JEFF_ROW(zn66, 30,  66),
    JEFF_ROW(zn68, 30,  68),
    JEFF_ROW(cd113, 48, 113),
    JEFF_ROW(cd114, 48, 114),
    JEFF_ROW(w182,  74, 182),
    JEFF_ROW(w183,  74, 183),
    JEFF_ROW(w184,  74, 184),
    JEFF_ROW(w186,  74, 186),
    JEFF_ROW(au197, 79, 197),
    JEFF_ROW(pb208, 82, 208),
};
...
static double sigma_geo_mb(int a) {
    ...
    return OSH_M_PI * OSH_NEUTRON_GEO_R0_CM * OSH_NEUTRON_GEO_R0_CM
           * cbrt_a * cbrt_a * OSH_CM2_TO_MB;
}

static void lookup_tier2(struct osh_neutron_xsec *xsec,
    ...
    if (xsec->n_warned < OSH_NEUTRON_XSEC_MAX_WARNED) {
        ...
        if (!already) {
            OSH_DIAG_INFOF(xsec->diag,
                ...
                "using Tripathi + geometric fallback", z, a);
