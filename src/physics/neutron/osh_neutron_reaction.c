...
static void do_elastic(unsigned int a, double e_mev, double const dir[3],
                       struct osh_rng *rng,
    ...
    if (ev->neutron_e_mev > 0.0) {
        cos_theta_lab = (1.0 + af * cos_theta_cm)
                        / sqrt(af * af + 2.0 * af * cos_theta_cm + 1.0);
        sin_theta_lab = sqrt(fmax(0.0, 1.0 - cos_theta_lab * cos_theta_lab));
        osh_kinematics_rotate_dir_cos(dir, ev->neutron_dir,
                                      cos_theta_lab, sin_theta_lab, cos_phi, sin_phi);
    } else {
    ...
    if (a == 1u) {
        ...
        if (t_recoil > 0.0) {
            ...
            osh_kinematics_rotate_dir_cos(dir, sec->dir,
                                          cos_recoil, sin_recoil,
                                          -cos_phi, -sin_phi);
            sec->energy  = t_recoil;
...
static void do_charge_exchange(unsigned int z, unsigned int a,
                               int product_pdg, unsigned int z_res, unsigned int a_res,
                               double e_mev, double const dir[3],
                               struct osh_rng *rng,
                               struct osh_neutron_reaction_event *ev) {
    ...
    W = sqrt((OSH_PART_MASS_NEUTRON + m_target) * (OSH_PART_MASS_NEUTRON + m_target)
             + 2.0 * e_mev * m_target);
...
static void do_compound(unsigned int z, unsigned int a, double e_mev,
                        double const dir[3],
    ...
    osh_nuclear_compound_step(z_c, a_c, e_star, p_lab,
                              &handler->fbu, diag, rng, &fbu_ev);

    /* translate FBU/sink event into neutron reaction event */
    if (fbu_ev.kind == OSH_NUCLEAR_EVENT_ABSORB
        || fbu_ev.n_secondaries == 0u) {
    ...
    ev->local_deposit_mev = 0.0;
    ev->n_secondaries = fbu_ev.n_secondaries > OSH_NUCLEAR_MAX_SECONDARIES
                        ? OSH_NUCLEAR_MAX_SECONDARIES
                        : fbu_ev.n_secondaries;
...
void osh_neutron_reaction_sample(struct osh_neutron_xsec *xsec,
    ...
    cum += sig[chosen].np;
    if (xi2 < cum && z >= 1u) {
        do_charge_exchange(z, a, OSH_PART_PDG_PROTON,
                           z - 1u, a, e_mev, dir, rng, event_out);
        return;
    }
    ...
    if (xi2 < cum && z >= 2u && a >= 4u) {
        do_charge_exchange(z, a, OSH_PART_PDG_HE4,
                           z - 2u, a - 3u, e_mev, dir, rng, event_out);
        return;
    }
